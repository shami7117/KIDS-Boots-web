...
import EditModal from "./orderEdit";
...
const Index = () => {
    ...
    const { data: SellerData, isLoading: SellerLoading, isError: SellerError } = useQuery(
        ...
        async () => {
            ...
            const response = await OrderApi.getSellersOrders(userId, activeButton);
            return response;// Assuming your API returns data property

        }
    );
    const { data: lengthSellerData, isLoading: lengthSellerLoading, isError: lengthSellerError } = useQuery(
        ['Orders'],
        async () => {

            const response = await OrderApi.getAllSellersOrders(userId);
            return response;// Assuming your API returns data property
    ...
    const { data: BuyerData, isLoading: BuyerLoading, isError: BuyerError } = useQuery(
        ...
        async () => {
            ...
            const response = await OrderApi.getBuyersOrders(userId, activeButton);
            return response;// Assuming your API returns data property

        }
    );
    const { data: lengthBuyerData, isLoading: lengthBuyerLoading, isError: lengthBuyerError } = useQuery(
        ['Orders'],
        async () => {

            const response = await OrderApi.getAllBuyersOrders(userId);
            return response;// Assuming your API returns data property

        }
    );

    console.log("BUYER ORDERS", BuyerData)

    const timestampData = SellerData;
    ...
    console.log("cancelled:", lengthSellerData);



    const PickupTasks = sellerData !== null ? lengthSellerData?.filter((task) => task.status === "Pickup") : lengthBuyerData?.filter((task) => task.status === "Pickup");
    const CanceledTasks = sellerData !== null ? lengthSellerData?.filter((task) => task.status === "Cancelled") : lengthBuyerData?.filter((task) => task.status === "Cancelled");
    const CompletedTasks = sellerData !== null ? lengthSellerData?.filter((task) => task.status === "Completed") : lengthBuyerData?.filter((task) => task.status === "Completed");
    const NewTasks = sellerData !== null ? lengthSellerData?.filter((task) => task.status === "New") : lengthBuyerData?.filter((task) => task.status === "New");
                        ...
                        <table
                            ...
                            <thead className=" my-3 fontFamily  border-b border-[DFDFDF] uppercase">
                                <tr className="text-[#777777]  text-left px-4 py-2">
                                    ...
                                    {
                                        SellerData !== null && <th className="px-3 font-[500] text-center text-sm md:text-[14px]">
                                            Customer
                                        </th>
                                    }
                                    ...
                                    {
                                        SellerData !== null && <th className=" font-[500] text-center text-sm md:text-[14px]">
                                            Action
                                        </th>
                                    }
                                ...
                                {sellerData !== null ?
                                    SellerData?.map((order) => (
                                        <tr
                                            ...
                                            {
                                                SellerData !== null && <td className="font-[400] text-center ">
